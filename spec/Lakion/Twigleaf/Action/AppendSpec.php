 ...
 */
class AppendSpec extends ObjectBehavior
{
    function let()
    {
    ...
    {
        $this->shouldHaveType('Lakion\Twigleaf\Action\Append');
    }
    ...
    function it_appends_content_to_matched_elements(Crawler $template, Crawler $elements)
    {
        $template->filter('p.comment')->shouldBeCalled()->willReturn($elements);
        $elements->append('<a href="#">Foo</a>')->shouldBeCalled();
